...
import { CharacterEquipPicker, type CompatibleCharacter } from './OwnedInventoryView'
import './echo-inventory.css'

type SortKey = 'score' | 'newest' | 'name' | 'cost' | 'level'

const sortOptions: Array<{ value: SortKey; label: string }> = [
  { value: 'score', label: 'Roll grade' },
...
export function InventoryView({ echoes, builds: _builds = [], characters = [], refresh, openScanner, embedded = false }: { echoes: Echo[]; builds?: Build[]; characters?: OwnedCharacter[]; refresh: (scope?: 'all' | 'echoes' | 'echoes-builds') => Promise<void>; openScanner: () => void; embedded?: boolean }) {
  ...
  const reset = () => { setQuery(''); setCosts(echoCosts); setRarities(echoRarities); setSonatas([]); setMainStats([]); setSubStats([]); setLockState('all'); setAssignment('all'); setShowExcluded(false); setSort('score'); setDescending(true); setSortOpen(false); setOpenMultiFilter(null); setPage(1) }
  ...
  const characterOptions = useMemo(() => characters.flatMap((item): CompatibleCharacter[] => {
    const catalog = characterCatalog.find((entry) => entry.id === item.catalogId)
  ...
  const patchEcho = async (echo: Echo, patch: Partial<Echo>) => {
    if (echo.locked && patch.excluded) return
  ...
  return <section className="echo-inventory-view">
    ...
    {echoes.length > 0 && <Panel className="inventory-filter echo-inventory-toolbar">
      <div className="echo-toolbar-main">
        ...
        <button type="button" className={`echo-filter-toggle${filtersOpen ? ' active' : ''}`} aria-expanded={filtersOpen} onClick={() => { setSortOpen(false); setFiltersOpen((open) => { if (open) setOpenMultiFilter(null); return !open }) }}><Icon name="settings"/>Filters{activeFilterCount > 0 && <b>{activeFilterCount}</b>}</button>
        <div className={`echo-sort-control${sortOpen ? ' open' : ''}`} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setSortOpen(false) }} onKeyDown={(event) => { if (event.key === 'Escape') { setSortOpen(false); event.currentTarget.querySelector<HTMLButtonElement>('.echo-sort-trigger')?.focus() } }}>
          <div className="echo-sort-picker">
            <button type="button" className="echo-sort-trigger" aria-label="Sort Echoes" aria-haspopup="true" aria-expanded={sortOpen} onClick={() => setSortOpen((open) => !open)}><span>{sortOptions.find((option) => option.value === sort)?.label}</span><strong aria-hidden="true">⌄</strong></button>
            {sortOpen && <div className="echo-sort-menu" aria-label="Sort Echoes">{sortOptions.map((option) => <button type="button" aria-pressed={sort === option.value} className={sort === option.value ? 'active' : ''} onClick={() => { setSort(option.value); setSortOpen(false) }} key={option.value}><span>{option.label}</span><i aria-hidden="true">{sort === option.value ? '✓' : ''}</i></button>)}</div>}
          </div>
          <button type="button" className="echo-sort-direction" aria-label={descending ? 'Sort descending' : 'Sort ascending'} title={descending ? 'Descending' : 'Ascending'} onClick={() => setDescending((value) => !value)}>{descending ? '↓' : '↑'}</button>
        </div>
        <button type="button" className="primary echo-toolbar-add" onClick={openScanner}><Icon name="plus"/>Add Echo</button>
      </div>
