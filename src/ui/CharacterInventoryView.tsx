...
export function CharacterInventory({ owned, weapons = [], echoes = [], builds = [], equippedLoadouts = [], theorycraftBuilds = [], settings, roverGender, refresh, characterIdentifier, onCharacterChange }: CharacterInventoryProps) {
  ...
  return <>
    ...
    <Panel className="owned-filter chip-toolbar character-roster-toolbar"><label className="search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search characters..."/><span className="search-count" aria-live="polite">{visible.length}/{owned.length}</span></label><FilterChips label="Element" hideLabel values={characterElements} selected={elements} onChange={setElements} renderValue={(value) => <ElementFilterIcon element={value}/>}/><FilterChips label="Rarity" hideLabel values={characterRarities} selected={rarities} onChange={setRarities} renderValue={(value) => `${value} ★`}/><button type="button" className="primary" onClick={() => setPickerOpen(true)}><Icon name="plus"/>Add character</button></Panel>
    <div className="character-candy-grid">{visible.map(({ item, catalog, weapon, weaponEntry, equipped }) => {
      ...
      return <article className={`character-candy-card rarity-${catalog.rarity}`} key={item.id} role="button" tabIndex={0} onClick={openCharacter} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') openCharacter() }}>
        <button className={item.favorite ? 'favorite active' : 'favorite'} aria-label={item.favorite ? 'Remove from favorites' : 'Add to favorites'} onClick={async (event) => { event.stopPropagation(); await db.characters.update(item.id, { favorite: !item.favorite }); await refresh() }}/>
