...
describe('Echo substat integrity', () => {
  const stats = [{ key: 'critRate' as const }, { key: 'atk' as const }, { key: 'critRate' as const }]
  ...
  it('keeps the current row key while excluding keys used by other rows', () => {
    expect(availableSubstatKeys(['critRate', 'atk', 'critDamage'], stats.slice(0, 2), 0)).toEqual(['critRate', 'critDamage'])
  })
